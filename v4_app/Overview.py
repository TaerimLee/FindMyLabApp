"""
FindMyLab — Overview page (v4 multipage app).

Run with:
    streamlit run v4_app/Overview.py

v4 changes vs. v3 (see conversation for full list):
- Plotly HTML downloads keep their colors (color-scheme fix).
- Plotly PNG downloads now use `fig.to_image()` (kaleido) via an explicit
  "Download image" button, instead of the modebar camera-icon screenshot.
- `requirements.txt` regenerated from the current `find_my_lab` env.
- Professor Prioritization page: "Include pre-prints" toggle for the full
  paper list (Table 4).
"""
...
from ui_helpers_v4 import dataframe_download_button, disable_clear_cache_shortcut
