"""FindMyLab v2 — Page 3: Professor Prioritization."""

import streamlit as st

from core_loader import core
from ui_helpers import plotly_download_button, dataframe_download_button
...
with st.sidebar:
    ...
    top_n_papers = st.slider(
        "Top N papers (overall ranking, Table 1)", 5, 100, core.DEFAULT_TOPN_PAPERS
    )
...
if run_button or "pp_results" in st.session_state:
    if run_button:
        with st.spinner(
            ...
            model = _load_model()
            df_similarity = core.compute_similarity(
                df_paper, embeddings, keyword, model
            )

            table1 = core.get_table1_top_papers(
                df_similarity, year_filter, top_n_papers
            )
            table2, df_top_papers_by_professor, df_paper_year = (
        ...
        st.session_state["pp_results"] = dict(
            ...
            table1=table1,
            table2=table2,
            df_top_papers_by_professor=df_top_papers_by_professor,
            df_paper_year=df_paper_year,
            top_n_pis=top_n_pis,
        )
    ...
    df_top_papers_by_professor = results["df_top_papers_by_professor"]
    df_paper_year = results["df_paper_year"]
    ...
    if table2.empty:
        ...
        st.subheader("Table 1 — Top matching papers overall")
        ...
        st.subheader("Table 2 — Top professors by aggregated similarity")
        st.dataframe(table2, use_container_width=True, hide_index=True)
        dataframe_download_button(
            table2, "Download (CSV)", "Table2_top_professors.csv", key="dl_t2"
        )

        st.divider()

        st.subheader("Figure 6 — Similarity distribution per top professor")
        ...
        st.subheader("Table 3 — Top professors' top papers")
        ...
        st.subheader("Table 4 — Full paper list for top professors")
        st.dataframe(table4, use_container_width=True, height=500, hide_index=True)
        dataframe_download_button(
